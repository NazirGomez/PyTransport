...
def directory(NC):
    dir = Path(os.path.dirname(r'C:\Users\nazir\Downloads\PyTransport')).parent.absolute()
...
def pathSet():
    dir = os.path.dirname(r'C:\Users\nazir\Downloads\PyTransport')
...
def compileName(name, NC=False):
    ...
    subprocess.run(['pip uninstall -y '+name_module], shell=True) 
    ...
    for line in ln:
        if line.startswith('# Define the extension module'):
            ...
            f.write(f'    sources=["{extension1}","{extension2}"],\n')
    ...
    loc = '../../'
    loc = Path(os.path.dirname(r'C:\Users\nazir\Downloads\PyTransport')).resolve().parents[0]
